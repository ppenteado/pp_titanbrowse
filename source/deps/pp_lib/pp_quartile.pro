...
;-
function pp_quartile,iarr,quart,cut=cut,nan=nan,index=index,sort=s,normalize=normalize
compile_opt idl2,logical_predicate
  nan=n_elements(nan) ? nan : 1B
  if nan then begin
    w=where(finite(iarr),complement=wc,ncomplement=nwc)
    arr=iarr[w]
  endif else arr=iarr
  s=sort(arr)
  narr=n_elements(arr)
  ...
  ret=keyword_set(cut) ? q[1]<arr>q[0] : q
  if keyword_set(cut) && nwc then ret[wc]=!values.d_nan
  if keyword_set(normalize) then begin
    mr=minmax(ret,/nan)
    ret=(ret-mr[0])/(mr[1]-mr[0])
  endif
  return,ret
end
