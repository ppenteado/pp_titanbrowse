...
function pp_locate,array,histogram=hh,$
  ...
  sort_indices=s,reverse_sort=sr,use_pointers=use_pointers,counts=counts,_extra=ex
...
if mhh then hh=hash(_strict_extra=ex)
...
last=0ULL
ret=use_pointers ? {keys:uarray,values:ptrarr(n_elements(uarray))} : hash(_strict_extra=ex)
if make_counts then counts=use_pointers ? {keys:uarray,values:0LL} : hash(_strict_extra=ex)
