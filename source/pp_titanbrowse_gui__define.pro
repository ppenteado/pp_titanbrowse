...
self.version='201901'
;Initialize db
self.db=obj_new('pp_titanbrowse',mdbfiles=mdbfiles,vis=vis,_strict_extra=_ex)
...
;maps[0].name='none'
maps=['none',rpath+'earth_bmr.tiff',rpath+'issmap_201506.tiff',rpath+'vimsmap_2009.tiff',rpath+'issmap_2009.tiff','GIBS']
tab4=obj_new('pp_mapwidget',tabw,name='map',title='Map',scr_xsize=DEFXSZ,scr_ysize=DEFYSZ-150,maps=maps,dbobject=self.db)
...
;Core bands
  ...
  nbands=strtrim(sindgen(std.bands),2)+' ('+string(*std.wavs,format='(F7.4)')+$
    string(181B)+'m '+string(1d4/(*std.wavs),format='(F7.2)')+' cm-1)'
;Backplanes
